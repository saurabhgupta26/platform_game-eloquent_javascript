...
DOMDisplay.prototype.syncState = function (state) {
  if (this.actorLayer) this.actorLayer.remove(); //removing the actor player from the view.
  this.actorLayer = drawActors(state.actors); //creating the fresh actor
  this.dom.appendChild(this.actorLayer); //appending to the dom layer
...
State.prototype.update = function (time, keys) {
  ...
  if (newState.status != "playing") return newState; //if the player is not playing as in touched lava
  ...
  for (let actor of actors) {
    if (actor != player && overlap(actor, player)) {
      newState = actor.collide(newState); //if the actor(can be anything, wall, lava or a nothing below, hence will fall) overlaps with the player, it will call a function collide
    }
  }
...
function trackKeys(keys) {
  let down = Object.create(null); //this function is tracking the keys and updating the display accordingly
